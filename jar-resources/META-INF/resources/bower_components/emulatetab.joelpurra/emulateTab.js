...
(function (document, $, namespace, pluginName) {
    ...
    var eventNamespace = "." + pluginName,
        focusable = ":input, a[href],:button,.btn,[type=submit],.dropdown",
        ...
        internal = {
            ...
            emulateTabbing: function ($from, offset) {
                ...
                $next.focus();
                //next tab logic goes here


                //data-plus-as-tab="false"
                // For some browsers, `attr` is undefined; for others,
                // `attr` is false.  Check for both.
                var attr = $next.attr('data-plus-as-tab');
                if (typeof attr === typeof undefined)
                    attr = "nope";

                if (attr === "false") {
                    var nextId = $next.attr("id");
                    var isDisabled = $next.prop('disabled');

                    if (!isDisabled)
                        $next.trigger("click"); //trigger submit elements
                    if (!isDisabled)
                        angular.element($next.attr("id")).triggerHandler('click');
